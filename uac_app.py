...
#                 return
    ...
    #     runnable: EndToEndRouter = cl.user_session.get("runnable")
    #     if runnable is None:
    #         logger.error("Runnable is not initialized. Calling setup_runnable() again.")
    #         setup_runnable()
    #         runnable = cl.user_session.get("runnable")
    #         if runnable is None:
    #             await cl.Message(content="Lỗi hệ thống. Vui lòng thử lại sau!").send()
    #             return
...
#         # print("Message content uac: ", res.content)
...
def header_auth_callback(headers: Dict) -> Optional[cl.User]:
    print("Headers received for auth:", headers)  # Debug xem có gì trong headers
    email = headers.get("email")
    if email:
        return cl.User(identifier=email, metadata={"role": "user", "provider": "header"})
    else:
        return cl.User(identifier="anonymous", metadata={"role": "guest", "provider": "default"})
...
#     # Nếu chưa có phone_number, tạo user_id mặc định
#     if not phone_number:
...
#     # Tìm trong database, nếu chưa có thì tạo mới
#     user_info = user_info_management.find_one(query={"phone_number": phone_number})

#     if user_info is None and phone_number:
#         user_info_management.insert_one({
#             "phone_number": phone_number,
#             "user_id": user_id,
#             "user_name": user_name,
...
#                 update_data["email"] = email
#             if update_data:
#                 user_info_management.update_one(query={"phone_number": phone_number}, update_data=update_data)

#     logger.success(f"Login account: Name: {user_name}, Phone number: {phone_number}, Email: {email}")

#     runnable = EndToEndRouter(config=config, embedding_fn=embedding_fn, rerank_model=None, user_id=user_id)
#     cl.user_session.set("runnable", runnable)
...
async def on_chat_start():
    ...
    cl.user_session.set("summary_memory", [])
    cl.user_session.set("thread", {"id": str(uuid.uuid4()), "name": "Phiên tư vấn mới"})
    cl.user_session.set("identifier", "anonymous")  # Mặc định chưa login

    await cl.Message(
        content=(
            "Xin chào! Mình là DUTChat, Chatbot hỗ trợ tư vấn giải đáp thông tin của Trường Đại học Bách Khoa Đà Nẵng.\n\n"
            "Bạn vui lòng nhập **tên, số điện thoại và email** của bạn để bắt đầu nhé."
        )
    ).send()
...
async def on_message(message: cl.Message):
    ...
    runnable: EndToEndRouter = cl.user_session.get("runnable")
    awaiting_info = cl.user_session.get("awaiting_info", False)

    if awaiting_info:
        info = await info_extractor.analyze_the_response(USER_INFO_EXTRACTOR_SYSTEM, message.content)

        if not any(info.get(k, "") for k in ["phone_number", "user_name", "email"]):
            await cl.Message(
                content="Bạn vui lòng nhập tên, số điện thoại và email chính xác để bắt đầu nhé."
            ).send()
            return

        # Lưu từng thông tin hợp lệ
        for key in ["phone_number", "email", "user_name"]:
            value = info.get(key, "").strip()
            if value:
                if key == "phone_number":
                    if not is_vietnamese_phone_number(value):
                        await cl.Message(content="Số điện thoại không hợp lệ").send()
                        return
                if key == "email":
                    if not is_valid_email(value):
                        await cl.Message(content="Email không hợp lệ").send()
                        return
                cl.user_session.set(key, value)

        # Kiểm tra đủ thông tin
        if all(cl.user_session.get(k, "") for k in ["phone_number", "email", "user_name"]):
            cl.user_session.set("awaiting_info", False)
            # Cập nhật identifier để dùng trong suốt session
            cl.user_session.set("identifier", cl.user_session.get("email"))

            # Cập nhật tên thread
            thread = cl.user_session.get("thread", {})
            thread["name"] = f"Tư vấn - {cl.user_session.get('user_name')}"
            cl.user_session.set("thread", thread)

            setup_runnable()

            await cl.Message(content="Cảm ơn bạn đã cung cấp thông tin. Bây giờ bạn có thể đặt câu hỏi để mình tư vấn cho bạn nhé.").send()
            return
        else:
            await cl.Message(content="Bạn vui lòng nhập đầy đủ tên, số điện thoại và email để mình tiếp tục nhé.").send()
            return
    ...
    #     res = cl.Message(content="", type="system_message")
    #     await res.stream_token("Đang xử lý ...")
    ...
    #         await res.send()
        ...
        summary_memory.extend(memory[-2:])
        MAX_MEMORY_LENGTH = 20
        if len(memory) > MAX_MEMORY_LENGTH:
            memory = memory[-MAX_MEMORY_LENGTH:]
        if len(summary_memory) > MAX_MEMORY_LENGTH:
            summary_memory = summary_memory[-MAX_MEMORY_LENGTH:]
